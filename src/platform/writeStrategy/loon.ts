...
export class LoonWriteStrategy extends BaseRuleStrategy {
  ...
  writeMITMModeRule(fileName: string, ruleSet: Record<RuleSetNames, Array<string>>) {
    ...
    const mitmDomains: Set<string> = new Set([
      '*.beacon.qq.com',
      '*.qq.com',
      '*.gdt.qq.com',
      'ad*.sina.com',
      ...
      'b.zhuishushenqi.com',
    ]) // Special case for parsed failed
    ...
    dedupedUrlRegexPaths.forEach((i) => {
      ...
      if (result) {
        ...
        if (!mitmDomains.has(i.processed))
    ...
    const platform = this.getName()
    if (parsedFailures.size > 0)
      console.error('parse error', parsedFailures)

    if (mitmDomains.size === 0)
      return
    writeFileSync(`${this.outputDir}/${platform}/rule/${fileName}_hostname.plugin`, `#!name=DreamyTZK ${fileName} MITM HostName
#!desc=MITM HostName for Surge update:${new Date().toISOString()}
#!category=DreamyTZK 
#!author=@kkfive

[MITM]
hostname = ${Array.from(mitmDomains).join(', ')}`)
  }
}
